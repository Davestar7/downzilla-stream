    ...
    try {
        ...
        const outPut = new Promise(async (resolve, reject) => {
            ...
            if (type === "video") {
              ...
              const argss = ['--verbose', '--ignore-config', '--cookies', cookie, '--js-runtimes', 'node', '--no-warnings', '--skip-download', '--no-check-certificate', '--no-playlist', '--force-ipv4', '--retries', 'infinite', '--fragment-retries', 'infinite', '--no-cache-dir', '--extractor-args', 'youtube:player_client=web', '-J', url];
